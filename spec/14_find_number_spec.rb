...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min and max both equal 3' do
      subject(:tied_game) {described_class.new(3,3, number_guessing)}
      it 'returns 3' do
        guess =tied_game.make_guess
  ...
  describe '#game_over?' do
    ...
    let(:final_number) { double('random_number', value: 8) }
    subject(:game_result) { described_class.new(0, 9, final_number, 8) }
      ...
      it 'is game over' do
        expect(game_result).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject(:game_result) { described_class.new(0, 9, final_number, 3) }

      it 'is not game over' do
        expect(game_result).to_not be_game_over
      end
    end
  end
