...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:fav_drink) { described_class.new('limeade') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(fav_drink.type).to eql('limeade')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(fav_drink.ounces).to eql(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      subject(:my_espresson_drink) { described_class.new('colada')}
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_espresson_drink).to be_full
      end
    end

    context 'when drink has less than 16 ounces' do
      subject(:expresso_drink) { described_class.new('flat white', 10)}
      ...
      # remove the 'x' before running this test
      it 'is not full' do
        expect(expresso_drink).not_to be_full
      end
    end
  end
end
